...
"$PYTHON3" ${TOOL_DIR}/idifference.py --summary ${SAMPLE_DIR}/difference_test_[01].xml > idifference_test.txt

#Generate XML output.
"$PYTHON3" ${TOOL_DIR}/idifference.py --xml idifference_test.dfxml ${SAMPLE_DIR}/difference_test_[01].xml
if [ ! -x "$XMLLINT" ]; then
...
"$PYTHON3" ${TOOL_DIR}/cat_fileobjects.py --debug idifference_test.dfxml > idifference_test_cat.dfxml
