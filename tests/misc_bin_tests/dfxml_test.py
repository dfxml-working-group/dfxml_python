...
def test_all():
    ...
    assert (
        0.0
        == dftime(a_pacific_dftime.iso8601()).timestamp() - a_pacific_dftime.timestamp()
    )
    ...
    assert (
        re.sub(
            rx_xmlns,
            "",
            """<fileobject xmlns:dfxml="http://www.forensicswiki.org/wiki/Category:Digital_Forensics_XML">""",
        )
        == "<fileobject>"
    )
    assert (
        re.sub(rx_xmlns, "", """<fileobject delta:new_file="1">""")
    ...
    assert (
        re.sub(
            rx_xmlns,
            "",
            """<fileobject xmlns="http://www.forensicswiki.org/wiki/Category:Digital_Forensics_XML" attr="1">""",
        )
        == """<fileobject attr="1">"""
    )
