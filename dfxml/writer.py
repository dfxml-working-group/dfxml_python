...
class DFXMLWriter:
    ...
    def add_DFXML_execution_environment(self, e):
        ...
        try:
            ...
            ET.SubElement(ee, "boot_time").text = datetime.datetime.fromtimestamp(
                psutil.boot_time()
            ).isoformat()
        except ImportError:
            pass
        ...
        for name, value in os.environ.items():
            ET.SubElement(
                env,
                "var",
                {"name": escape(name, entities), "value": escape(value, entities)},
            )
    ...
    def timestamp(self, name):
        ...
        ET.SubElement(
            self.doc,
            "timestamp",
            {"name": name, "delta": str(now - self.tlast), "total": str(now - self.t0)},
        )
        self.tlast = now
    ...
    def add_loadavg(self, node):
        ...
        ET.SubElement(
            node,
            "loadavg",
            {"avg1": str(avgs[0]), "avg5": str(avgs[1]), "avg15": str(avgs[2])},
        )
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ET.SubElement(
                            ...
                            "memory_info",
                            {
                                "rss": str(mem.rss),
                                "vms": str(mem.vms),
                                "pfaults": str(mem.pfaults),
                                "pageins": str(mem.pageins),
                            },
                        )
                    except AttributeError:
                        ET.SubElement(
                    ...
                    ET.SubElement(
                        proc,
                        "cpu_times",
                        {"user": str(cpu.user), "system": str(cpu.system)},
                    )
    ...
    def add_spark(self, node):
        ...
        try:
            ...
            ET.SubElement(spark, "error").text = (
                "SPARK_ENV_LOADED present but requests module not available"
            )
            return
        ...
        if port >= p2:
            ET.SubElement(spark, "error").text = (
                f"SPARK_ENV_LOADED present but no listener on {host} ports {p1}-{p2}"
            )
            return
...
if __name__ == "__main__":
    ...
    arg_parser.add_argument(
        "--debug",
        help="Print the output. Default unless --write is specified",
        action="store_true",
    )
