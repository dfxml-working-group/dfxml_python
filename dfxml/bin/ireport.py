...
def process_files(fn):
    ...
    for fstype in fstypes:
        for ftype in ["jpg", "pdf", "doc", "txt"]:
            ...
            for i in filter(
                (lambda f: f.ext() == ftype and f.fragments == 2), all_files
            ):
                runs = False
                if hasattr(i, "block_runs"):
                    runs = i.block_runs
                if hasattr(i, "sector_runs"):
                    runs = i.sector_runs
                if not runs:
                    continue
            ...
            if len1stats.count() > 0:
                ...
                print(
                    "len1 average: %f stddev: %f"
                    % (len1stats.average(), len1stats.stddev())
                )
                print(
                    "len2 average: %f stddev: %f"
                    % (len2stats.average(), len2stats.stddev())
                )
    ...
    hist_exts.topn = 20
    for i in all_files:
        if i.size > 0 and i.fragments > 0:
            hist_exts.add(i.ext(), i.size)
    tab = table()
    tab.header = "File extension popularity and average size (suppressing 0-len files)"
    tab.col_headings = ["Ext", "Count", "Average Size", "Max", "Std Dev"]
    tab.omit_row = [[0, ""]]
    ...
    for i in all_files:
        if i.fragments > 0:
            hist_exts.add(i.ext(), i.fragments)
    tab = table()
