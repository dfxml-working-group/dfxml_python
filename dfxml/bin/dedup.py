...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--prefix", type=str, help="Only output files with the given prefix"
    )
    parser.add_argument(
        "--distinct", action="store_true", help="Report the distinct files"
    )
    parser.add_argument(
        "--dups",
        action="store_true",
    ...
    print(
        "Total files: {:,}  total MD5s processed: {:,}  Unique MD5s: {:,}".format(
            dobj.files, dobj.md5s, len(dobj.seen)
        )
    )
    ...
    if args.distinct:
        ...
        def report_distinct(names):
            if args.prefix and not names[0].startswith(args.prefix):
                return
