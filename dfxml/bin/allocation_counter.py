...
def main():
    ...
    for event, obj in Objects.iterparse(args.input_image):
        if isinstance(obj, Objects.FileObject):
            if (
                args.ignore_virtual_files
                and dfxml.bin.make_differential_dfxml.ignorable_name(obj.filename)
            ):
                continue
            ...
            if args.debug and obj.alloc_inode is None and obj.alloc_name is None:
                ...
                _logger.debug(
                    "Previous object: %s." % ET.tostring(prev_obj.to_Element())
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Use the same file-ignoring rules as make_differential_dfxml.py.",
    )
    parser.add_argument(
        "-d", "--debug", action="store_true", help="Enable debug printing."
    )
