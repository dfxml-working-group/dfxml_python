...
class DiskSet:
    ...
    def print_dups_report(self):
        ...
        dups = filter(
            lambda a: len(a[1]) > 1,
            self.fi_by_md5.items(),
        )
        dup_bytes = 0
        for md5hash, fis in sorted(
            dups, key=lambda a: a[1][0].filesize(), reverse=True
        ):
            for fi in fis:
...
def dfxml_info(fn):
    ...
    print(
        "{}    {} {:>7}   {:>10}".format(
            command_line, start_time, elapsed_seconds, memory_usage
        )
    )
    ...
    if timers:
        ...
        for name in times:
            ...
            print(
                fmt.format(
                    name, len(data), min(data), statistics.median(data), max(data)
                )
            )
