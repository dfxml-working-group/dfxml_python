...
class xml:
    ...
    def provenance(self):
        global args
        if args.allprovenance or args.commandline or args.pythonversion:
            self.push("creator")
    ...
    while True:
        ...
        if args.piecewise:
            ...
            if args.sha1:
                ...
                x.write(
                    "<hashdigest type='SHA1'>%s</hashdigest>" % sha1_part.hexdigest()
                )
            ...
            if args.sha256:
                ...
                x.write(
                    "<hashdigest type='SHA256'>%s</hashdigest>"
                    % sha256_part.hexdigest()
                )
...
def extract(fn):
    ...
    def start_element(name, attr):
        ...
        if name == "hashdigest":
            ...
            if (
                (kind == "MD5" and args.md5)
                or (kind == "SHA1" and args.sha1)
                or (kind == "SHA256" and args.sha256)
            ):
                cdata = ""
        else:
            cdata = None
...
"""
    parser.add_argument(
        "--piecewise", help="Specifies size of piecewise hashes", default=0, type=int
    )
    parser.add_argument(
        "--addfixml", help="Specifies XML to add to each file object (for labeling)"
    )
    ...
    parser.add_argument(
        "--iso-8601",
        help="Format timestamps as ISO-8601 in metadata",
        action="store_true",
    )
    parser.add_argument(
        "--nometadata",
        help="Do not include file metadata (times & size) in XML",
        action="store_true",
    )
    parser.add_argument(
        "--nofilenames", help="Do not include filenames in XML", action="store_true"
    )
    parser.add_argument(
        "--stripprefix",
        help='Remove matching prefix string from filenames (e.g. "/mnt/diskname" would reduce "/mnt/diskname/foo" to "/foo", and would not affect "/run/mnt/diskname/foo")',
    )
    parser.add_argument(
        "--stripleaddirs",
        help='Remove N leading directories from filenames (e.g. 1 would reduce "/mnt/diskname/foo" to "mnt/diskname/foo", 2 would reduce the same to "diskname/foo")',
        default=0,
        type=int,
    )
    parser.add_argument(
        "--includedirs",
        help="Include directories alongside files in file system walk output",
        action="store_true",
    )

    provenance_group = parser.add_argument_group(
        "provenance", "Options to record execution environment details in the output."
    )
    provenance_group.add_argument(
        "--allprovenance",
        help="Include all provenance information requestable in this option group",
        action="store_true",
    )
    provenance_group.add_argument(
        "--commandline", help="Record command line in output", action="store_true"
    )
    provenance_group.add_argument(
        "--pythonversion", help="Record Python version in output", action="store_true"
    )
