...
def extract_files(
    ...
    base_manifest.add_creator_library(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Do not write files to disk.  Only verifies computed vs. stored checksums of file content.",
    )
    parser.add_argument(
        "-x",
        "--xml",
        help="Pre-computed DFXML file.  If not supplied, Fiwalk is called on the image argument.",
    )
    parser.add_argument(
        "-k",
        "--keep-going",
        action="store_true",
        help="If a SleuthKit process error is encountered in extracting any file (note: this excludes checksum mismatches), the extraction halts unless this flag is passed.",
    )
    parser.add_argument(
        "--output-manifest",
        help="Path for recording DFXML manifest of all extracted files.",
    )
    parser.add_argument(
        "--error-manifest",
        help="Path for recording DFXML manifest of only files extracted with errors.",
    )
    parser.add_argument(
        "image", help="Subject disk image from which files will be extracted."
    )
    parser.add_argument(
        "output_directory", help="Target output directory.  Can already exist."
    )
