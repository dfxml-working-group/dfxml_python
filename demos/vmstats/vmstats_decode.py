...
def get_stats(root):
    ...
    return {
        "start_time": dateutil.parser.parse(
            root.find(".//execution_environment/start_time").text
        ),
...
def get_processes(root):
    ...
    for proc in root.findall(".//processlist/process"):
        rec = {"pid": proc.get("pid"), "name": proc.get("name")}
    ...
    def keyfunc(rec):
        ...
        if (
            "user" in rec
            and type(rec["user"]) == float
            and "system" in rec
            and type(rec["system"]) == float
        ):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--json", help="Render JSON file and JavaScript HTML files into a new directory"
    )
    ...
    for i in range(len(roots)):
        ...
        if args.html:
            with open(os.path.join(path, html_filename(root)), "w") as f:
                ...
                f.write(
                    html_generate(root, prev_fname=prev_fname, next_fname=next_fname)
                )

        if args.json:
            # Make the JSON file
            data = [
                {"stats": get_stats(root), "processes": get_processes(root)}
                for root in roots
            ]
            ...
            # Now clean the data...
            for slot in data:
                slot["stats"]["start_time"] = slot["stats"]["start_time"].isoformat()[
                    0:19
                ]
                slot["processes"] = [
                    p
            ...
            json_data = json.dumps(data, default=myconverter)
            print(
                "json shrunk by {} to {}".format(
                    json_data_len - len(json_data), len(json_data)
                )
            )
